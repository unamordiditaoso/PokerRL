...
def estimate_equity(hero_str, board_str=None, num_opponents=4, iters=20000, seed=None):
    ...
    for i in range(iters):
        deck = Deck()  # mazo estándar (52)
        # remover hero y board del mazo
        # Deck.peek/pop don't support removal by Card int, so creamos un mazo nuevo sin esas cartas
        used = set(hero + board)
        # build a deck list excluding used
        full_deck = [c for c in deck.cards if c not in used]
        # barajar
        random.shuffle(full_deck)

        # repartir cartas a oponentes
        opps = []
        idx = 0
        ...
        # idx += cards_needed  # no hace falta despues

        # evaluar manos (treys: menor score => mejor mano)
...
if __name__ == "__main__":
    ...
    # HERO: As Ks (As = A♠, Ks = K♠)
    # BOARD parcial: 2h 7d Jc
    hero = ["As", "Ks"]
    board = []
    opponents = 4   # num_opponents = 4 => mesa con 5 jugadores (hero + 4)
